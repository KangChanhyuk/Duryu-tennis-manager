...
.vs-circle {
    ...
    margin:4px auto; box-shadow:0 2px 8px rgba(229,57,53,.4);
}
.rnd-hdr {
    ...
    for ti,g in enumerate(gnames):
        with tabs[ti]:
            ...
            with mc:
                ...
                for m in matches:
                    t1,t2=tuple(m["t1"]),tuple(m["t2"]); s1,s2=int(m["s1"]),int(m["s2"])
                    if s1>0 or s2>0:
            ...
            for mi,m in enumerate(matches):
                ...
                with ca:
                    ...
                    s1=st.number_input(n1,0,50,int(m["s1"]),key=f"{tid}{g}{mi}s1",label_visibility="collapsed")
                with cb:
                ...
                with cc:
                    ...
                    s2=st.number_input(n2,0,50,int(m["s2"]),key=f"{tid}{g}{mi}s2",label_visibility="collapsed")
                if s1!=int(m["s1"]) or s2!=int(m["s2"]):
                    tour["groups"][g]["matches"][mi]["s1"]=s1
                    tour["groups"][g]["matches"][mi]["s2"]=s2
                    changed=True
            if changed:
                tours[tid]=tour; save_tours(tours); st.toast("✅ 점수 저장됨!")
...
# ══════════════════════════════════════════════════════════════
    ...
    with adm[2]:
        # ★ 엑셀 업로드 → 회원명부로 저장
        st.markdown('<div class="sec">📥 랭킹 엑셀 업로드 (회원명부 자동 저장)</div>', unsafe_allow_html=True)
        st.caption("업로드하면 랭킹 데이터와 함께 회원명부(이름 순서)가 자동 저장됩니다.")
        ...
        if up:
            try:
                ...
                if st.button("💾 랭킹 저장 + 회원명부 등록", type="primary", use_container_width=True):
                    save_rank(df_up)
                    # ★ 회원명부 저장 (랭킹순 이름 리스트)
                    if "이름" in df_up.columns:
                        save_members(df_up["이름"].astype(str).str.strip().tolist())
                        st.success(f"✅ 랭킹 저장 + 회원명부 {len(df_up)}명 등록 완료!")
                    else:
                        st.success("✅ 랭킹 저장 완료! (이름 컬럼 없어 회원명부 미등록)")
                    st.rerun()
        ...
        roster=load_members()
        if roster:
            st.markdown(f'<div class="sec">현재 회원명부 ({len(roster)}명)</div>', unsafe_allow_html=True)
            tags="".join([f'<span class="p-tag">{n}</span>' for n in roster])
        ...
        if df_cur.empty:
            ...
            with c2:
                st.download_button("📥 랭킹 다운로드", data=to_excel(edited),
                    file_name=f"두류랭킹_{date.today()}.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    use_container_width=True)
